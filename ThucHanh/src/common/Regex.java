...
public class Regex {
    ...
    public static final String REGEX_MA_SO = "^(STK-)\\d{4}$";

    public static String regexMaSoSo() {
        String maSoSo = CheckException.checkString();
        while (!maSoSo.matches(REGEX_MA_SO)) {
            System.out.println("Nhập không đúng định dạng mã dịch vụ !!" +
                    "\nYêu cầu nhập lại lại đây theo định dạng STK-xxxx: ");
            maSoSo = CheckException.checkString();
        }
        return maSoSo;
    }
